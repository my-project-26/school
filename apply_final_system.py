...
header { background-color: var(--primary-color); color: white; padding: 0.8rem 1rem; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
...
.math-problem { font-size: 3rem; font-weight: bold; margin: 1rem 0 1.5rem 0; display: flex; justify-content: center; align-items: center; gap: 0.8rem; }
.input-display { background: var(--bg-color); border: 3px solid var(--primary-color); border-radius: 12px; padding: 0.2rem 1rem; min-width: 90px; color: var(--primary-color); }
.numpad-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.8rem; max-width: 320px; margin: 0 auto; }
.btn-num { background-color: var(--bg-color); border: 2px solid #CBD5E1; border-radius: 14px; font-size: 1.8rem; font-weight: bold; color: var(--text-color); min-height: 60px; cursor: pointer; }
...
// Dynamic Content Loader (Lädt Ollama content.json wenn vorhanden)
...
fetch('content.json')
    ...
    .then(data => {
        if (data.klasse3_wortarten && data.klasse3_wortarten.length > 0) {
            ...
            console.log("🟢 Dynamische Ollama-Inhalte geladen.");
        }
    })
    .catch(() => console.log("⚪ Nutze integrierten Fallback-Datenpool."));
...
function switchTab(activeTab, activeModule) {
    [tabK1Anlaut, tabK1Silben, tabK1Zehner, tabK3Math, tabK3Lang].forEach(t => t.classList.remove('active'));
    [moduleK1Anlaut, moduleK1Silben, moduleK1Zehner, moduleK3Math, moduleK3Lang].forEach(m => m.classList.add('hidden'));
...
# 4. SW.JS
sw_content = """const CACHE_NAME = 'schul-app-v3';
const ASSETS = [
  './',
  './index.html',
  './style.css',
...
print("✨ Gesamtsystem inkl. Zehnerfeld & Ollama-Loader erfolgreich aktualisiert!")
