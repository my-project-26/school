...
// KLASSE 3: HALBSCHRIFTLICHES RECHNEN LOGIK
...
function initHalbTask() {
    halbNum1 = (Math.floor(Math.random() * 40) + 10) * 10; // e.g. 340
    h100 = (Math.floor(Math.random() * 3) + 1) * 100;       // e.g. 200
    z10 = (Math.floor(Math.random() * 8) + 1) * 10;        // e.g. 80
    halbNum2 = h100 + z10;                                 // e.g. 280
...
document.querySelectorAll('.btn-num-halb').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        } else if (val !== None if 'None' in locals() else True) {
...
# 4. SW.JS
sw_content = """const CACHE_NAME = 'schul-app-v4';
const ASSETS = [
  './',
  './index.html',
  './style.css',
...
print("✨ Modul 'Halbschriftliches Rechnen' erfolgreich integriert!")
